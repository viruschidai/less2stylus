...
(function() {
  var Parser, baseVisitor, crypto, defineVisitor, expressionVisitor, extend, findRootAncestor, fs, funcMap, genVar, graph, isNamespaceDefinition, isString, main, mixinMap, mixins, mixinsClass, path, renderMixinArg, renderMixinParam, renderPrelude, renderTree, renderValue, sortTree, sortingVisitor, toUnquoted, tree, treeVisitor, util, _ref, _ref1;
  ...
  sortTree = function(node, parent) {
    ...
    impl.parent = parent;
    return node = new tree.visitor(impl).visit(node);
  };
  ...
  findRootAncestor = function(node, parent) {
    var a;
  ...
  sortingVisitor = Object.create({
    ...
    visitRuleset: function(node, options, directive) {
      var namespace, rule, _i, _j, _len, _len1, _ref2, _ref3;
      if (directive == null) {
        directive = '';
      }
      if (!node.root) {
    ...
    visitMixinCall: function(node, options) {
      ...
      graph.push({
        left: this.parent,
        right: findRootAncestor(node, this.parent)
      });
      if (node["arguments"].length > 0) {
        v = "" + (renderValue(node.selector).slice(1));
        v += "(" + (node["arguments"].map(renderMixinArg).join(', ')) + ")";
  ...
  main = function() {
    ...
    return parser.parse(str, function(err, node) {
      ...
      console.log('mixins ----------------');
      return console.log(util.inspect(mixins, {
        depth: 10
      }));
    });
  };
